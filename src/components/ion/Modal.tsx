...
const ModalHeader = ({
  ...
  <div
    className={clsx('flex gap-2 p-5', {
      'flex-row items-center justify-start': iconLocation === 'left' && align === 'left',
      'flex-row items-center justify-center': iconLocation === 'left' && align === 'center',
      'flex-col items-start justify-center': iconLocation === 'top' && align === 'left',
      'flex-col items-center justify-center': iconLocation === 'top' && align === 'center',
...
>(({ className, ...props }, ref) => (
  <ModalPrimitive.Description ref={ref} className={clsx('text-sm text-sub-foreground', className)} {...props} />
));
...
const Modal = React.forwardRef<React.ElementRef<typeof ModalPrimitive.Root>, ModalProps>(
  ({ showClose, trigger, className, title, subtitle, icon, iconLocation, align, bordered, footer, ...props }, ref) => (
    <ModalPrimitive.Root {...props}>
      ...
      <ModalContent ref={ref} className={clsx('sm:min-w-96 w-full max-w-lg overflow-hidden', className)}>
        {showClose && (
          ...
          >
            <Button iconLeading={<X size={16} />} emphasis="low" color="secondary" />
          </ModalPrimitive.Close>
        )}
        {(title || subtitle || icon) && (
          <ModalHeader
...
export { ModalPortal, ModalOverlay, ModalClose, ModalTrigger, ModalContent, ModalHeader, ModalTitle, ModalDescription };
